...
export default class ApprovalActions extends Vue {
  ...
  get showStudentApproval() {
    ...
    if (this.entity.campaignStatus.status.indexOf("APPROVED_STUDENTS") > -1)
      return false;
  ...
  get showEmployeeApproval() {
    ...
    if (!this.isActive) return false;
    if (this.entity.campaignStatus.status.indexOf("DENIED") > -1) return false;
    if (this.entity.campaignStatus.status.indexOf("APPROVED_EMPLOYEES") > -1)
      return false;
  ...
  showElement(ev, element) {
    ...
    el.find(targetElement).toggleClass("show");

    let isVisible = el.find(targetElement).hasClass("show");
    if (isVisible) {
      el.find(targetElement)
        .find(".dropdown-menu")
        .toggleClass("show");
