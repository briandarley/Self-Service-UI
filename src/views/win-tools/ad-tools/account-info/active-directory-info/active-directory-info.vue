  ...
  <div class="pb-100" v-if="data">
    ...
    <div class="account-details row-odd">
      ...
      <div>
        ...
        <span v-if="data.dualRoleUser">{{
          (data.dualRoleUser.primaryDesignation > 0) | toYesNo
        }}</span>
        <span v-else>No</span>
      </div>
      <div v-if="data.dualRoleUser && data.dualRoleUser.primaryDesignation > 0">
        <div>
          ...
          <span>{{
            data.dualRoleUser.primaryDesignation | toPrimaryDesignation
          }}</span>
        </div>
        <div>
    ...
    <div class="account-details  row-even" v-if="data.userDetail">
      ...
      <div>
        ...
        <div>
          <div v-for="item in data.userDetail.proxyAddresses" :key="item">
            {{ item }}
          </div>
        </div>
      </div>
    ...
    <div class="account-details  row-odd" v-if="data.userDetail">
      ...
      <div>
        ...
        <span v-else
          >{{ data.userDetail.lockoutTime | formatDateTime }}

          <button class="btn  btn-outline-danger" @click="enableAccount()">
            <i class="fa fa-chevron-right"></i> Enable
          </button>
        </span>
      </div>
      <div class="recent-locks">
        <span class="label">Recent Lockouts</span>
        <div v-if="data.splunkLockouts.totalRecords == 0">
          No Recent Lockouts
        </div>
        <div v-else>
          <div>
            Total Lockouts (Past 90 Days):
            {{ data.splunkLockouts.totalRecords | formatNumber }}
          </div>
      ...
      <div>
        ...
        <span v-if="!isEmpty(data.userDetail.accountExpiresDate)">{{
          data.userDetail.accountExpiresDate | formatDateTime
        }}</span>
      ...
      <div v-if="data.userDetail.pwdSet">
        ...
        <span v-if="!isEmpty(data.userDetail.pwdExpires)">{{
          data.userDetail.pwdExpires | formatDateTime
        }}</span>
        <span v-else></span>
      </div>
