...
export default class MyAliases extends BaseValidateMixin {
  ...
  get isUncDomainSelected() {
    
      return this.model.domain === "unc.edu";
    
  }

  get allowAdd() {
    if(this.model.domain === "unc.edu" && this.availableUncAliasEmailOptions.length){
      return true
    }
    return this.model.domain != "unc.edu";
  ...
  getEmailAddresses() {
    let proxyAddresses = this.userAdProfile.proxyAddresses.filter(
      (c) => !c.endsWith("adminliveunc.mail.onmicrosoft.com")
    );
  ...
  async loadProvisionProfile() {
    ...
    try {
      ...
      let values = await Promise.all([
        ...
        this.UserProfileService.getAvailableUncAliasEmailOptions(this.userId)
      ]);

      this.userLdapProfile = values[0];
      this.userAdProfile = values[1];
      this.allowedDomains = values[2];
      this.availableUncAliasEmailOptions = values[3];


      if (
        this.userAdProfile.status === false ||
  ...
  async updatePrimaryAlias() {
    ...
    try {
      ...
      await this.UserProfileService.updatePrimaryAlias(
        this.userId,
        primarySmtp.email
      );
  ...
  async _validateAliasEntry() {
    if(this.model.uncAliasSelection)
    {
      return true;
    }
    ...
    if (adUserProfile.status !== false) {
      this.toastService.error(
        "Requested alias has already been taken by another user. "
      );
      return false;
    }
    ...
    if (!this._underMaxAliasCount()) {
      this.toastService.error(
        "You have exceeded the maximum number of allowed aliases.  You must be below the maximum of 5 before being allowed to add another."
      );
      return false;
    }
  ...
  async addAlias() {
    ...
    let newAlias = this.model.uncAliasSelection || this.newAlias;

    
    this.spinnerService.show();
    try {
      let response = await this.UserProfileService.addEmailAlias(
        this.userId,
        newAlias
      );
      if (response.status === false) {
        this.toastService.error(
          `Failed to add alias. Error Response: ${response.message}`
        );
        return;
      }
